...
public class HandsOn03Test extends UnitContainerTestCase {
    ...
    public void test_orderByMemberStatusDisplayOrder() throws Exception {
        ...
        // Assert
        assertFalse(memberList.isEmpty()); //空確認
...
もともと一件しかなかった検索結果が「二件」になるはずです。 
    ...
    public void test_officialMemberDatetimeBetween20051001And20051003() throws Exception {
        ...
        // 0901memo: startとendの方が、beginとendよりも、馴染みがあるのでついつい使用仕してしまっていました🙇
        // リーダブルコードもう一回読み直しま🙇
        // 0901修正メモ: beginとendに変数名を変更した
        LocalDateTime targetBeginDatetime = LocalDateTime.of(2005, 10, 1, 0, 0);
        ...
        ListResultBean<Member> memberList = memberBhv.selectList(cb -> {
            ...
            cb.query().setFormalizedDatetime_FromTo(targetBeginDatetime, targetEndDatetime, op -> op.compareAsDate());
        });
        ...
        // _/_/_/_/_/_/_/_/
        // 0901修正メモ: 誤解を生まないように空チェックのコメントを横に書くように変更した
        assertFalse(memberList.isEmpty()); // 空チェック
        for (Member member : memberList) {
            // 会員名称と正式会員日時と会員ステータス名称をログに出力
            String memberName = member.getMemberName();
             ...
             *
             * [Advice]
             * To get non-specified column from entity is not allowd.
             * #nonSpecifiedColumn_access
             * Non-specified means using SpecifyColumn but the column is non-specified.
             * Mistake? Or specify the column by your condition-bean if you need it.
             * For example:
             * (x):
             ...
             * ... = member.getBirthdate(); // *NG: exception
             * });
             * (o):
             * memberBhv.selectEntity(cb -> {
             * cb.specify().columnMemberName();
             ...
             * }).alwaysPresent(member -> {
             * ... = member.getMemberName();
             * ... = member.getBirthdate(); // OK
             * });
             *
             * While, reluctantly you need to get the column without change conditions,
             * you can enable non-specified column access by the condition-bean option.
             * The method is cb.enable...()
             */
...
もともと増えたのであれば、なぜ増えたのか？が把握できていたらOKです。 
    ...
    ※2: 1974年12月31日生まれの人、1975年1月1日生まれの人。前者は検索に含まれて、後者は含まれない。テストデータに存在しない、もしくは、存在に依存するのがためらうほどのピンポイントのデータは、自分で作っちゃうというのも一つの手。
    (エクササイズ 6 や7 でやっていた adjust がまさしくそれ: 同じように adjustXxx()という感じでprivateメソッドにしましょう)
    ※3: 1 から 9 までの任意の会員IDを選び updateNonstrict()してみましょう。まあ、一桁代の会員IDが存在すること自体への依存は割り切りで。
    最低限それだけのテストデータで用意されていないとお話にならないってことで。
